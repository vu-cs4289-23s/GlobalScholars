...
import {
  getPostByIdAsyncAction,
  getPostsByUserAsyncAction,
...
const tabs = [{ title: 'My Posts' }, { title: 'Saved Posts' }];
...
export default function ProfilePage() {
  ...
  return (
    <div
      id="profile-page"
      className="flex overflow-x-hidden h-screen w-screen bg-blue-light"
    >
      <SideBar />
      <div className="w-full h-screen -mx-2 flex justify-center">
        ...
        <div className="absolute top-40 z-1 w-[75%] overflow-scroll h-[80%] sm:h-[78%]">
          ...
          <div className="flex justify-center rounded-lg bg-white mx-4 p-2 px-4 my-4 sm:flex-row sm:mx-20">
            <div
              id="my_posts"
              className="flex border-black rounded-lg border-2 p-2 m-auto hover:bg-gray-200"
            ...
            >
              ...
              <div className="font-bold text-lg ">Saved Posts</div>
            </div>
          </div>
          <div>{myPosts ? 'Posts' : 'Saves'}</div>
            ...
            <div>
              {posts.map((post, index) => (
                <ForumPost
                  ...
                  date={post.timestamp}
                  url={
                    post.location && post.location.city !== 'Accra'
                      ? post.location.city
                      : post.program
